...
<body>
  ...
  <script>
    (function(){
      ...
      function recordStartupError(message){
        if(window.PDF_COURSE_STARTUP_ERROR)return;
        window.PDF_COURSE_STARTUP_ERROR=message;
        showRuntimeError(message);
      }
      window.addEventListener('error',function(event){
        var target=event.target;
        if(target&&target.tagName==='SCRIPT'){
          recordStartupError('Script failed to load: '+(target.getAttribute('src')||'unknown script'));
          return;
        }
        recordStartupError(event.message||'Unknown script error.');
      });
      window.addEventListener('unhandledrejection',function(event){
        var reason=event.reason;
        recordStartupError(reason&&reason.message?reason.message:String(reason||'Unknown script error.'));
      });
      window.setTimeout(function(){
        var status=document.getElementById('status');
        if(status&&status.textContent==='Loading PDF...'){
          recordStartupError('The PDF.js runtime did not finish loading.');
        }
      },12000);
    })();
...
const runtime = `const pdfjsLib = globalThis.pdfjsLib;
